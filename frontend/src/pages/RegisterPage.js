...
export default function RegisterPage() {
  ...
  const s = {
    ...
    input: { width:'100%', padding:'0.65rem 0.875rem', border:'1.5px solid #E2E8F0', borderRadius:9, fontSize:'0.875rem', outline:'none', fontFamily:'inherit' },
  ...
  return (
    <div style={s.page}>
      <div style={s.card}>
        ...
        <p style={s.sub}>Start your financial journey today</p>
        <form onSubmit={handleSubmit}>
          {[['name','Full Name','text','John Doe'],['email','Email Address','email','you@example.com'],['password','Password','password','Min 8 characters']].map(([key, label, type, placeholder]) => (
            <div style={s.group} key={key}>
              <label style={s.label}>{label}</label>
              <input style={s.input} type={type} required value={form[key]} placeholder={placeholder}
